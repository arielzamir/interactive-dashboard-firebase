...
async function seedTrafficStats() {
  const batch = db.batch();

  trafficStats.forEach((stat) => {
    const docRef = db.collection("trafficStats").doc(stat.date);
    batch.set(docRef, stat);
  });

  await batch.commit();
  console.log(
    `Seeded ${trafficStats.length} documents to 'trafficStats' collection.`
  );
  process.exit(0);
}

seedTrafficStats().catch(console.error);
